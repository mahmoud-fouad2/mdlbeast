...
const MAX_UPLOAD_BYTES = Number(process.env.UPLOAD_MAX_BYTES || (10 * 1024 * 1024)) // default 10MB
...
const upload = multer({ storage, limits: { fileSize: MAX_UPLOAD_BYTES }, fileFilter: (_req: any, file: any, cb: any) => {
  if (file.mimetype !== 'application/pdf') return cb(new Error('Only PDF allowed'))
  cb(null, true)
}})

// protect uploads route
router.use(authenticateToken)
    ...
    if (!jwtLike && !sbPrefixed) {
      console.error('Supabase service key invalid format; aborting upload.')
      if (inProd) return res.status(400).json({ error: 'SUPABASE_SERVICE_ROLE_KEY does not look like a Service Role JWT or sb_secret_ key. Replace with the Service Role Key from Supabase settings.' })
  ...
  try {
    ...
    if (useR2) {
      try {
        ...
        const ext = path.extname(f.originalname || f.filename || '') || ''
        const safeBase = `${user.tenant_id || 'global'}/${Date.now()}-${Math.random().toString(36).slice(2,9)}`
        const key = `uploads/${safeBase}${ext}`
        const rs = fs.createReadStream(f.path)
        const url = await uploadBuffer(key, rs as any, f.mimetype, 'public, max-age=0')
        try { fs.unlinkSync(f.path) } catch (e) {}
        return res.json({ url, name: f.originalname, size: stat.size, storage: 'r2', key, bucket: process.env.CF_R2_BUCKET || null })
      } catch (e: any) {
        console.error('R2 upload failed')
        if (inProd) return res.status(500).json({ error: 'R2 upload failed' })
        const tenantDir = path.join(uploadsDir, String(user.tenant_id || 'public'))
        if (!fs.existsSync(tenantDir)) fs.mkdirSync(tenantDir, { recursive: true })
        const outPath = path.join(tenantDir, f.filename)
        fs.renameSync(f.path, outPath)
        const url = `/uploads/${user.tenant_id || 'public'}/${f.filename}`
        return res.json({ url, name: f.originalname, size: stat.size, storage: 'local' })
      }
    }

    if (!supabaseUrl || !supabaseKey || !supabaseBucket) {
      const msg = 'Supabase storage not configured.'
      console.error(msg)
    ...
    // Validate service role key quickly
    const jwtLike = /^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/.test(supabaseKey)
    const sbPrefixed = supabaseKey.startsWith('sb_secret_')
    if (!jwtLike && !sbPrefixed) {
      try { fs.unlinkSync(f.path) } catch (e) {}
      if (inProd) return res.status(400).json({ error: 'SUPABASE_SERVICE_ROLE_KEY does not look valid' })
      const tenantDir = path.join(uploadsDir, String(user.tenant_id || 'public'))
      if (!fs.existsSync(tenantDir)) fs.mkdirSync(tenantDir, { recursive: true })
      const outPath = path.join(tenantDir, f.filename)
      fs.renameSync(f.path, outPath)
    ...
    try {
      ...
      try { fs.unlinkSync(f.path) } catch (e) {}
      return res.json({ url, name: f.originalname, size: stat.size, storage: 'supabase', key })
    } catch (e: any) {
      console.error('Supabase upload failed')
      if (inProd) return res.status(500).json({ error: 'Supabase upload failed' })
      const tenantDir = path.join(uploadsDir, String(user.tenant_id || 'public'))
      if (!fs.existsSync(tenantDir)) fs.mkdirSync(tenantDir, { recursive: true })
      const outPath = path.join(tenantDir, f.filename)
      fs.renameSync(f.path, outPath)
    ...
    console.error('Upload error:', String(err?.message || err))
    res.status(500).json({ error: err.message || 'Upload failed' })
  }
})
