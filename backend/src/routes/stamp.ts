...
router.use(authenticateToken, requirePermission('archive', 'stamp'))
...
router.post('/:barcode/stamp', async (req, res) => {
  try {
    ...
    const permissions = (req as any)._effectivePermissions
    const canViewAll = permissions?.archive?.view_all === true
    const canViewOwn = permissions?.archive?.view_own === true

    const isOwner = doc.user_id && user?.id && Number(doc.user_id) === Number(user.id)
    if (!canViewAll && !(canViewOwn && isOwner)) {
      return res.status(403).json({ error: 'لا تملك الصلاحية للوصول لهذا المستند' })
    }
    ...
    // fetch pdf bytes (support R2 or local uploads only - Supabase removed)
    let pdfBytes: Buffer

    // Load storage config: USE_R2_ONLY is always true (project migrated to R2-only)
    ...
    try {
      ...
      if (useR2 && (pdf.key || (pdf.url && String(pdf.url).startsWith((process.env.CF_R2_ENDPOINT || '').replace(/\/$/, ''))))) {
        ...
        // R2-only mode - return error if no R2 match found
        if (!useR2) {
          console.error('Stamp: server is configured R2-only but R2 is not configured or URL does not match R2 endpoint')
    ...
    // Otherwise, save to storage (R2 or local only - Supabase removed)
    // Overwrite original file when possible (R2 key or local file), otherwise create a new local file and replace the first attachment
    try {
      let targetKey = pdf.key
      let targetBucket = pdf.bucket || process.env.CF_R2_BUCKET || ''

      // If we have an R2 key/bucket, attempt to overwrite the same object
      const useR2 = String(process.env.STORAGE_PROVIDER || '').toLowerCase() === 'r2' || Boolean(process.env.CF_R2_ENDPOINT)

      if (useR2 && targetKey && targetBucket) {
        try {
          ...
          attachments[targetIndex] = { ...(attachments[targetIndex] || {}), url: signedUrl || (getPublicUrl(finalKey) || ''), size: outBuf.length, key: finalKey, bucket: targetBucket, stampedAt }
